...
const PDFContent = ({ entries, totalAmount, date, estimateNumber, sellerName }) => {
  ...
  useEffect(() => {
    const generateQRCode = async () => {
      try {
        // Create a summary of the receipt data
        const receiptData = {
          estimateNumber: entries[0]?.estimate_number,
          sellerName,
          date: formattedDate,
          time: currentTime,
          items: entries.map(entry => ({
            ...
            netWeight: entry.total_weight,
            amount: entry.rate_amt,
          })),
        };
    ...
    generateQRCode();
  }, [entries, sellerName, formattedDate, currentTime]);
